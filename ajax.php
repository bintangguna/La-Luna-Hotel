    ...
    if (!$email && !$password) {
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                $_SESSION['staff_type'] = $user['staff_type_id'];
                if ($user['staff_type_id'] == 3) {
...
if (isset($_POST['register'])) {
    ...
    if (!$email && !$password) {
        ...
        $query = "INSERT INTO `staff` (`emp_id`, `emp_name`, `staff_type_id`, `shift_id`, `id_card_type`, `id_card_no`, `address`, `contact_no`, `salary`, `joining_date`, `updated_at`, `username`, `password`) VALUES (NULL, '$email', '3', '', '', '', '', '', '', current_timestamp(), current_timestamp(), '$username', '$password');";
        $result = mysqli_query($db->connection, $query);
        if ($result) {
            $_SESSION['username'] = $user['username'];
            $_SESSION['emp_id'] = mysqli_insert_id($db->connection);
            header('Location:index.php?reservation');
            // header('Location:login.php?empty')
        } else {
            header('Location:login.php?loginE');
...
if (isset($_POST['add_room'])) {
    ...
    if ($room_no != '') {
        ...
        if (mysqli_num_rows(mysqli_query($db->connection, $sql)) >= 1) {
            $response['done'] = false;
...
if (isset($_POST['booking'])) {
    ...
    $customer_result = mysqli_query($db->connection, $customer_sql);
    ...
    if ($customer_result) {
        ...
        $booking_result = mysqli_query($db->connection, $booking_sql);
        if ($booking_result) {
            $room_stats_sql = "UPDATE room SET status = '1' WHERE room_id = '$room_id'";
            if (mysqli_query($db->connection, $room_stats_sql)) {
                $response['done'] = true;
...
if (isset($_POST['check_in_room'])) {
    ...
    if ($booking_id != '') {
        ...
        $result = mysqli_query($db->connection, $updateBooking);
        if ($result) {
            $updateRoom = "UPDATE room SET check_in_status = '1' WHERE room_id = '$room_id'";
            $updateResult = mysqli_query($db->connection, $updateRoom);
            if ($updateResult) {
                $response['done'] = true;
            } else {
                $response['done'] = false;
...
if (isset($_POST['username'])) {
    ...
    if ($staff_type == '' && $shift == '' && $salary == '') {
        ...
        $customer_sql = "INSERT INTO staff (`emp_id`, `emp_name`, `staff_type_id`, `shift_id`, `id_card_type`, `id_card_no`, `address`, `contact_no`, `username`, `password`) VALUES ('$name', '$staff_type', '$shift', '$id_card_id', '$id_card_no', '$address', '$contact_no', '$salary', '$username', '$password')";
        $customer_result = mysqli_query($db->connection, $customer_sql);
        if ($customer_result) {
            $response['done'] = true;
            $response['data'] = 'Successfully Booking';
...
if (isset($_POST['change_shift'])) {
    ...
    $insert_result = mysqli_query($db->connection, $insert);
    // && $update_result  
    if ($result) {
